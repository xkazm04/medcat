...
**Replace simple message rendering:**
Instead of just rendering getMessageText(), iterate over message.parts and switch on part.type:

```tsx
{message.parts.map((part, partIndex) => {
  switch (part.type) {
    case 'text':
      return <MessageBubble key={partIndex} content={part.text} role={message.role} />;
